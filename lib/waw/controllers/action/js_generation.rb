  ...
  class ActionController < ::Waw::Controller
    ...
    class JSGeneration
      ...
      def run
        ...
        static = Waw.find_rack_app {|app| ::Waw::StaticController===s}
        ...
        # And let's go now!
        file = File.join(folder, "#{name}_generated.js")
      ...
      def generate_js(buffer)
        ...
        ActionController.controllers.sort{|c1, c2| c1.class.name <=> c2.class.name}.each do |controller|
          ...
          url = Waw.find_url_for(controller)
          error("Unable to find the mapping URL for #{controller} (required for code generation)")
